...
| 25.2 | **Tarix — savdo va to'lov BITTA xronologik oqimda**, ikkita alohida massiv emas | Bu tarix, ya'ni "bu odam nima oldi va nima to'ladi" degan savolga xronologik javob. Ikkita massiv bo'lsa, ularni ekranda baribir birlashtirish kerak bo'lardi va tartiblash mantiqi frontendga ko'chardi — server bilan ekran o'rtasida ikkinchi haqiqat manbai. **Sahifalash:** ikkita jadval birlashtirilgani uchun kursor `(vaqt, id)` juftligini olib yuradi, faqat vaqtni emas. Bu ixtiyoriy ehtiyot chorasi emas: `sale-confirmation.service.ts` boshlang'ich to'lovni `paidAt: soldAt` bilan yozadi, ya'ni HAR BIR nasiya savdosida savdo va to'lov aynan bir xil vaqt tamg'asiga ega bo'ladi va faqat vaqt bo'yicha kursor sahifa chegarasida qatorni takrorlardi. `id` ikkala jadval bo'ylab taqqoslanadi — UUID global yagona, ya'ni tartib to'liq |
| 25.3 | **Tarixda `DRAFT` yo'q; `REVERSAL` va bekor qilingan savdo esa BOR** | `DRAFT` hech narsaga ta'sir qilmagan (§7.7) — uni ko'rsatish "bu odam nimadir oldi" degan yolg'on taassurot berardi. `REVERSAL` va `CANCELLED` esa kiradi: ular aylanmaga qanday kirishi §24.1 da hal qilingan, lekin bu yerda savol boshqa. **Tarix — hisobot emas.** Hisobotda bir summani ikki marta sanash haqiqiy xavf; tarixda esa savdo va uning qaytarilishi IKKI XIL PAYTDAGI IKKI XIL HODISA, takrorlanish emas. Asl qator `RETURNED` deb tursa ham, u QACHON qaytarilganini ayta olmaydi — "yanvarda oldi, martda qaytardi" esa aynan §6.9 ("Ehtiyot bo'ling") uchun ega qaraydigan naqsh. Ishorani `sales.total` olib yuradi (§22.2), ya'ni qatorda qo'shimcha hisob kerak emas; `reversal_kind` DTO'ga qo'shiladi, chunki "Qaytarish" bilan "Bekor qilish" ni faqat `status` bo'yicha ajratib bo'lmaydi |
...
| 25.10 | **Yangi FK uchun sukut bo'yicha javob — KOMPOZIT (`[xId, shopId] -> [id, shopId]`)**; bir ustunli FK istisno bo'lib, sabab va tasnif talab qiladi. Ro'yxat `tenant-fk-audit.spec.ts` da qotirilgan | T-01 dan keyin to'liq inventarizatsiya qilindi: schema'da shop-scoped modelga ishora qiluvchi 33 ta bir ustunli FK bor, kompozit esa 6 ta. Auditda **ochiq teshik topilmadi** — 6 tasi DB trigger bilan (bazada empirik sinaldi: cross-tenant `INSERT` rad etiladi), 9 tasi servis tekshiruvi bilan, 8 tasi server tomonidan hisoblanadi, 10 tasi hali ishlatilmaydi yoki moduli yozilmagan. Lekin servisga tayangan 9 tasi **intizomga** bog'liq, T-01 esa aynan shundan kelib chiqqan edi: servis tekshiradi, baza kafolat bermaydi. 33 tasini ko'r-ko'rona migratsiya qilish ish ko'rsatish bo'lardi, xavfsizlik emas — shuning uchun tasnif kodga muzlatildi. Yangi bir ustunli FK qo'shilsa test yiqiladi va muallifni yo kompozit qilishga, yo sababini yozib tasniflashga majbur qiladi (§21.8 dagi "xavfli istisnolar ro'yxati" naqshi) |
| 25.11 | **`FileAsset` ga ishora qiluvchi 6 ta FK `Storage` moduli ulanishidan OLDIN kompozitga o'tkaziladi** — 10-bosqichning birinchi vazifasi | Ular hozir "xavfsiz" emas, "hali sinovdan o'tmagan": modul ulanmagani uchun baholab bo'lmaydi. Ular orasida `Customer.passportFileId` — tizimdagi yagona PII tashuvchi FK. Fayl biriktirish endpointi RLS tekshiruvisiz yozilsa, Shop A Shop B mijozining pasport suratini o'ziga bog'lay oladi; bu IDOR emas, **shaxsiy hujjatning sizib chiqishi** (§15.5 faylni hech qachon ochiq havolada saqlamaslikni talab qiladi). Hozir tuzatish arzon — ma'lumot ham, servis kodi ham yo'q; modul ulangandan keyin esa u "keyin topilib tuzatiladigan" T-01 ga aylanadi. Ma'lum qiyinchilik: oltalasi ham `onDelete: SetNull`, ya'ni `notification_logs` dagi ustun-ro'yxatli `SET NULL (ustun)` shakli kerak (§25.9 da bir marta hal qilingan) |

> **§25.3 qayta ko'rib chiqildi (2026-08-16).** Dastlabki tahrir `REVERSAL`
> ni tarixdan CHIQARIB tashlagan edi, sababi: "uning fakti asl qatorning
> `status` i orqali ko'rinadi". Bu §24.1 dagi hisobot mulohazasini tarixga
> noo'rin ko'chirish edi — ikki marta sanash hisobotda xavf, tarixda esa
> ikki hodisa ikki marta emas, ikki marta sodir bo'lgan. Qaror teskarisiga
> o'zgartirildi.
>
