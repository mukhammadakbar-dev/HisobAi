...
| #    | Qaror | Sabab |
|------|-------|-------|
| 18.1 | **Mahsulot rasmi (§4.10) 3-bosqichda yo'q** — u 10-bosqichda `Storage` moduli bilan birga keladi | §4.10 MinIO'ni talab qiladi, `StorageProvider` porti esa hali yozilmagan va §22 uni 10-bosqichga (shartnoma PDF'i bilan birga) qo'ygan. Bitta ixtiyoriy maydon uchun MinIO o'rnatish, vaqtinchalik havola (§15.5) va fayl validatsiyasini katalog bosqichiga tortish — bosqichni ikki barobar kattalashtiradi. Rasmsiz mahsulot to'liq ishlaydi; `products.image_file_id` ustuni schema'da **allaqachon bor**, ya'ni keyin qo'shish bitta forma maydoni, migratsiya emas |
...
| 19.7 | **Do'kon logosi (§3.6) 10-bosqichda** — `Storage` moduli bilan birga. `settings.logo_file_id` ustuni va `FileKind.SHOP_LOGO` schema'da allaqachon bor; `PATCH /settings` uni **qabul qilmaydi** | §18.1 (mahsulot rasmi) va §19.2 (passport rasmi) bilan aynan bir xil sabab: rasm MinIO'ni, vaqtinchalik havolani (§15.5) va fayl validatsiyasini (`API.md` §7) talab qiladi. Logosiz do'kon sozlamalari to'liq ishlaydi. Bu qator ataylab yozilmoqda: qolgan ikkita rasm kechiktirilgani hujjatda bor edi, logo esa yo'q edi — ya'ni u "unutilgan" bo'lib ko'rinardi |
...
| 20.5 | **Valyuta ayirboshlash (§11.6) UI'siz qoldi** — `POST /cashbook/exchange` bor, ekrani yo'q | §22 uni "Kengaytirish" (11-bosqich) ro'yxatiga qo'ygan, MVP-1 ga emas. Endpoint kassa modulining ichki mantiqi bilan birga yozilgani uchun qoldirildi (uni olib tashlash `CashExchange` jadvalini ham yetim qilardi), lekin ekran qo'shilmadi: ayirboshlash kunlik ish emas va usiz kassa to'liq ishlaydi |
...
| 20.9 | **Kassa yozuviga ilova (chek surati, §11.9) qo'shilmadi** | §18.1, §19.2 va §19.7 bilan bir xil sabab: fayl `Storage` modulini talab qiladi va u 10-bosqichda. `cash_entries` da fayl ustuni schema'da allaqachon bor, ya'ni keyin qo'shish forma maydoni bo'ladi, migratsiya emas |
